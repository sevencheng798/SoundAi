 ...
 */
#ifndef __FFMPEGDECODER_H_
#define __FFMPEGDECODER_H_
...
using LayoutMask = int64_t;		// add Sven
...
public:
    ...
    static std::unique_ptr<FFmpegDecoder> create(	
        ...
        const PlaybackConfiguration& outputConfig);	 // add last param
     ...
     *
     * @note: The second option is a work around for() FFmpeg doesn't seem to be handling @c EAGAIN as
...
private:
    ...
    FFmpegDecoder(
        std::unique_ptr<FFmpegInputControllerInterface> inputController,
        AVSampleFormat format,		// add Sven
        LayoutMask layout,			// add
        int sampleRate);			// add
    ...
    public:
         ...
         * @param sampleRate The output sample rate in Hz.
         */
        UnreadData(AVSampleFormat format, LayoutMask layout, int sampleRate); // add Sven params
    ...
    const AVSampleFormat m_outputFormat;		//add 

    /// The output channel layout.
    const LayoutMask m_outputLayout;			// add 

    /// The output sample rate.
    const int m_outputRate;						// add Sven
